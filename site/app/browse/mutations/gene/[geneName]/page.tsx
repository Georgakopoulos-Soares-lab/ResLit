...
export default async function GeneMutationsPage({ params }: PageProps) {
  const { geneName } = await params
  const decodedName = decodeURIComponent(geneName)

  const [genePapers, mutations] = await Promise.all([
  ...
  // ── allele grouping (same logic as gene detail page) ──────────────────────
  ...
  // ── overview aggregates ───────────────────────────────────────────────────
  const primaryGene = genePapers[0] ?? null
  ...
  const mechanismClass =
    primaryGene?.resistance_mechanism_class ??
  ...
  // ── status: best across gene papers + mutations ───────────────────────────
  const statusPriority: Record<string, number> = { curated: 0, pending: 1, rejected: 2 }
  const allStatuses = [
    ...genePapers.map((r) => r.status),
    ...mutations.map((m) => m.status),
  ] as CurationStatus[]
  const bestStatus = allStatuses.reduce<CurationStatus>(
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        ...
        <div className="mb-8 rounded-xl border border-border/60 bg-gradient-to-br from-slate-50 via-white to-violet-50/50 p-6">
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3 mb-1">
            ...
            {primaryGene?.pmid && (
              <Button variant="outline" asChild className="shrink-0">
                <a
                  href={`https://pubmed.ncbi.nlm.nih.gov/${primaryGene.pmid.split(',')[0].trim()}`}
                  target="_blank"
          ...
          {primaryGene?.encodes && (
            <p className="text-muted-foreground mb-5">{primaryGene.encodes}</p>
          )}
          ...
          <div className="flex flex-wrap items-start justify-between gap-3 mb-5">
            <div className="flex flex-wrap gap-2">
              ...
              {primaryGene?.encodes && (
                <a
                  href={`https://www.uniprot.org/uniprotkb?query=${encodeURIComponent(primaryGene.encodes)}+AND+(taxonomy_id%3A2)`}
                  target="_blank" rel="noopener noreferrer"
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-xs text-amber-600 mt-0.5">Protein entries for the encoded enzyme</div>
                  </div>
                </a>
              )}
            </div>
              ...
              {allOrganisms.length > 0 && (
                <div>
                  ...
                  <dd className="text-base italic">
                    ...
                    {allOrganisms.length > 3 && (
                      <span className="not-italic text-muted-foreground"> +{allOrganisms.length - 3} more</span>
                    )}
                  </dd>
                </div>
              )}
        ...
        <div className="space-y-6">
                    ...
                    <tbody>
                      {alleleEntries.map((allele) => (
                        <tr key={allele.name} className="border-b last:border-0 hover:bg-muted/30">
                          <td className="px-4 py-3 font-medium font-mono">{allele.name}</td>
                          <td className="px-4 py-3 text-sm">{allele.paperCount}</td>
                          ...
                          <td className="px-4 py-3 text-muted-foreground">{allele.countries.join(', ') || '-'}</td>
                          <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">{allele.years.join(', ') || '-'}</td>
                          <td className="px-4 py-3"><StatusBadge status={allele.status} /></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          {/* Papers — combined selector for gene papers and/or mutation papers */}
