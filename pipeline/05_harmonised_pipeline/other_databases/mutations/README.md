...
The first version of this step was done in R: take `AMRFinder_coreGenes.txt`
(from the AMRFinder folder), `GenesPointFinders.txt` (from the ResFinder
folder), and `snps.txt` (from the CARD folder); for each file, copy any line
with multiple mutations so that each resulting line has exactly one mutation;
then run `concatenate_details.R` to produce a combined CSV.

`concatenate_details.py` (Step 2 below) is the Python rewrite of that
original R script and is the version actually used to produce the current
output files in this folder — `concatenate_details.R` itself is not included
here.
...
- `shortname_antibiotics.tsv` (from CARD, maps drug abbreviations to full names;
  located at `../../reference_data/card/`)
...
Input: `resfinder_pointMutations_expanded.xlsx`, `ReferenceGeneCatalog_PointMutations.csv`,
`snps.txt`, `shortname_antibiotics.tsv` (for CARD drug abbreviation lookup)
...
- A normalized gene name (`Gene_normalized`) is created for cross-database
  ...
  `re.sub(r"[^a-z0-9]", "", name.lower())`. This collapses variants like
  "gyrA_1", "#embB", "16S-rrsB", and "GyrA" into a single comparable key
  (e.g. "gyra1", "embb", "16srrsb", "gyra"), so the same gene can be matched
  across ResFinder, AMRFinder, and CARD despite different naming conventions.

**Step 3: Enrich with paper titles and publication years from PubMed**
Script: `../../shared_scripts/enrich_database_metadata.py` (shared enrichment script)
Input: `full_list_mutations.csv`
Output: `full_list_mutations_enriched.csv`

Command:
```
python3 ../../shared_scripts/enrich_database_metadata.py \
...
Input: `full_list_mutations_enriched.csv`, `../../reference_data/card/aro_index.tsv`
(for CARD ARO-to-DNA mapping)
...
Input: `Full_list_mutations_otherDatabases.csv`, `Bacteria_genes_all.txt`
(reference list of gene names — see "Not included" in the top-level
`harmonised_pipeline/README.md`)
...
- If not, `Nucleotide_Change` is cleared (the number was likely a copy of
...
- Per the original working notes, 180 rows were dropped this way (8,972 -> 8,792);
  the actual output file on disk has 8,866 rows (106 dropped from the 8,972-row
  input) — the discrepancy suggests undocumented manual post-processing happened
  after this note was last edited.

> Some further row changes after Step 9 were applied manually, not scripted.
